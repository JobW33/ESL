...
// Select 1
//#include "hw/hardware_manager_DE10.h"
...
void determineCounts(){
  ...
  int minPitchCount = _getCount(manager.pitch, -dutyCycle, uSleepTime); 
  int maxPitchCount = _getCount(manager.pitch, dutyCycle, uSleepTime);
  printf(" >>> Pitch count range: %d\n", maxPitchCount-minPitchCount);

  // Get the QD count value at both extremes of the yaw
  int minYawCount = _getCount(manager.yaw, -dutyCycle, uSleepTime); 
  int maxYawCount = _getCount(manager.yaw, dutyCycle, uSleepTime); 
...
void determineRotationSpeed(double dutyCycle){
  ...
  printf(" >>> Yaw @%.2f Duty cycle: Took %.3fms to move to the other side and %.3fms to move back.\n", dutyCycle, double(yawDurationForwards)/1000.0, double(yawDurationBackwards)/1000.0);
  ...
  printf(" >>> Pitch @%.2f Duty cycle: Took %.3fms to move to the other side and %.3fms to move back.\n", dutyCycle, double(pitchDurationForwward)/1000.0, double(pitchDurationBackwards)/1000.0);
}
